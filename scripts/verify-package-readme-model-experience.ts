...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/tenant/dsh-tenant-credentials-local': { kind: 'indirect', reason: 'The tenant-scoped fork only changes where a credential value is stored (per-tenant vs shared file); the consuming adapter still owns every model-facing use the value authorizes.' },
