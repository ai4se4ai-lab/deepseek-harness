/**
 * Tenant-scoped fork of `@deepseek-ai/dsh-sandbox-local`'s local sandbox
 * backend. Narrow, auditable diff against upstream (`packages/sandbox/sandbox-local/src/index.ts`):
 * every point that previously called `bwrapProfileArgs(policy)` /
 * `landlockProfileArgs(policy)` first resolves
 * `tenantRoot = dshHomePath('tenants', ctx.tenantContext.requireCurrent())` and
 * passes it through to this package's forked `./profiles.ts`. `requireCurrent()`
 * fails closed: with no tenant identity bound to the calling async chain,
 * `confine()` throws instead of building a profile at all — it never falls back
 * to an unconfined or upstream-shaped profile. A `workspace-write` policy whose
 * `workspaceRoot` falls outside the resolved tenant root is refused for the same
 * reason (defense in depth beneath `@mindportalix/dsh-tenant-session-guard`, which
 * is what is supposed to keep `workspaceRoot` inside the tenant root in the
 * first place — see `SandboxPolicyService.resolve`, which derives it from
 * `session.header.cwd`).
 *
 * Everything else — rung selection, probing, denial signatures, runner-failure
 * rules, the windows-acl write-grant management, platform detection, and error
 * handling — is copied verbatim from upstream and NOT the subject of this fork.
 * @module @mindportalix/dsh-tenant-sandbox-local
 */
...
/**
 * Whether `candidate` is `root` itself or a filesystem descendant of it.
 * @param root - absolute tenant root.
 * @param candidate - absolute path to check.
 * @returns whether `candidate` is `root` or nested under it.
 */
...
/**
 * Probe whether `bwrap` can create the profile; the provider caches the
 * bounded result. Uses the OS temp directory as the probe's tenant-root
 * placeholder — the probe is a one-time platform capability check unrelated
 * to any real tenant, and it must succeed (or fail) the same way regardless
 * of which tenant's request happens to trigger the first `confine()` call.
 */
function defaultProbeBwrap(timeoutMs: number): boolean {
  const probe = spawnSync('bwrap', [...bwrapProfileArgs({ mode: 'read-only', workspaceRoot: '/' }, tmpdir()), '--', 'true'], {
    timeout: timeoutMs,
    stdio: 'ignore',
  })
...
function assertPositiveFinite(name: string, value: number): void {
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error(`tenant-sandbox-local: ${name} must be a positive finite number`)
  }
}
...
/**
 * Tenant-scoped local process-sandbox provider. Registers as `ctx.sandbox`,
 * exactly like upstream `LocalSandboxProvider` (there is exactly one sandbox
 * provider per composition; this package's Cordis patch row replaces the
 * upstream `sandbox` row rather than adding a second one). See the module
 * doc comment for the tenant-resolution diff against upstream.
 */
export class TenantSandboxLocalProvider extends SandboxProvider {
  /** Required so a composition mounting this provider without the tenant-context service fails loud at boot, not at first confine(). */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (runner.length === 0 && runnerFailureSignatures.length > 0) {
      throw new Error('tenant-sandbox-local: runnerFailureSignatures requires runnerCommand')
    }
    if (runner.length > 0 && runnerFailureSignatures.length === 0) {
      throw new Error('tenant-sandbox-local: runnerCommand requires at least one runnerFailureSignatures entry')
    }
    if (runnerFailureSignatures.some(signature => signature.trim().length === 0 || /[\r\n]/u.test(signature))) {
      throw new Error('tenant-sandbox-local: runnerFailureSignatures entries must be non-empty single-line strings')
    }
  ...
  /**
   * Resolve the calling tenant's root, failing closed with no bound tenant
   * identity. A `workspace-write` policy whose `workspaceRoot` is outside
   * that root is refused rather than confined against it: the sandbox
   * provider is the last enforcement layer, and it must not trust an
   * upstream policy value it cannot itself verify.
   * @param policy - the resolved per-call policy.
   * @returns the calling tenant's absolute `$DSH_HOME/tenants/<tenantId>` root.
   * @throws when no tenant identity is bound, or `policy.workspaceRoot` escapes the tenant root.
   */
  private resolveTenantRoot(policy: SandboxPolicy): string {
    const tenantId = this.ctx.tenantContext.requireCurrent()
    const tenantRoot = dshHomePath('tenants', tenantId)
    if (policy.mode === 'workspace-write' && !isUnderRoot(tenantRoot, policy.workspaceRoot)) {
      throw new Error(
        `tenant-sandbox-local: workspace root ${JSON.stringify(policy.workspaceRoot)} is outside tenant root `
        + `${JSON.stringify(tenantRoot)}; refusing to build a write-capable sandbox profile`,
      )
    }
    return tenantRoot
  }
   ...
   *   `SANDBOX_UNAVAILABLE` error when the platform has no usable runner, or a
   *   plain `Error` when no tenant identity is bound or `policy.workspaceRoot`
   *   escapes the tenant root.
   */
  confine(argv: readonly string[], policy: SandboxPolicy): ConfinedArgv {
    const tenantRoot = this.resolveTenantRoot(policy)
    if (this.runnerCommand !== undefined) {
      return {
        argv: [...this.runnerCommand, ...bwrapProfileArgs(policy, tenantRoot), '--', ...argv],
        enforcement: 'full',
    ...
    const runnerArgv = this.runnerArgv(selected.runner, policy, tenantRoot)
    return {
  ...
  private runnerArgv(runner: SelectedRunner['runner'], policy: SandboxPolicy, tenantRoot: string): string[] {
    switch (runner) {
      case 'bwrap': return ['bwrap', ...bwrapProfileArgs(policy, tenantRoot)]
      case 'landlock': return [this.landlockLauncher(), ...landlockProfileArgs(policy, tenantRoot)]
  ...
  private materializeAclGrant(sessionId: SessionId, workspaceRoot: string): AclTempCapability {
    ...
    if (!this.workspaceGrants.has(workspaceRoot)) {
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError([error, cleanupError], 'tenant-sandbox-local windows-acl workspace grant failed and its cleanup also failed')
        }
        throw error
      }
    ...
    try {
      ...
      if (cleanupFailures.length > 0) {
        throw new AggregateError([error, ...cleanupFailures], 'tenant-sandbox-local windows-acl temp grant materialization failed and its cleanup also failed')
      }
      throw error
    }
  ...
  private revokeAclGrants(): void {
    ...
    if (failures.length > 0) {
      this.ctx.logger.warn(`tenant-sandbox-local: windows-acl grant cleanup completed with ${failures.length} failure(s)`)
...
export default TenantSandboxLocalProvider
