...
**MINDPORTALIX-TENANT-ISOLATION.** `ctx.connection.settingsAvailable` (surfaced to browser plugins as `ctx.remote.$host.settingsAvailable`) is a narrower derived state for the settings/credentials domain only: it equals `isLoopback` except when the served page carries the `__DSH_MP_SETTINGS_TRUSTED__` page global, which this package's node half declares through `webserver/index-inject` exactly when `ctx.get('tenantContext')` resolves — i.e. only under the tenant-isolation patch layer, where this process is reachable solely through the trusted MindPortalix DSH proxy. Native-path-open consumers must keep reading `isLoopback`, since a path this Host can open is meaningless on the browser's own, different machine; only the settings/credentials wire-availability decision may read `settingsAvailable`. Separately, when the tenant-isolation patch layer is mounted, this package's `/api` route binds the trusted proxy's tenant id (`ctx.tenantContext.run(...)`, resolved from the request headers) around the whole unary Remote dispatch, so every business Service's tenant-scoping wrapper (see `@mindportalix/dsh-tenant-session-guard`) sees the caller's identity.
