...
describe('connection node half', () => {
  ...
  it('declares no browser trust signal when the tenant-isolation patch layer is not mounted', async () => {
    ...
    expect(table).not.toContainEqual({ kind: 'global', name: '__DSH_MP_SETTINGS_TRUSTED__', value: true })
    await fiber.dispose()
  })
  ...
  it('declares the browser trust signal once the tenant-isolation patch layer resolves a tenant context', async () => {
    ...
    const table: IndexInjection[] = []
    ctx.emit('webserver/index-inject', table)
    expect(table).toContainEqual({ kind: 'global', name: '__DSH_MP_SETTINGS_TRUSTED__', value: true })
    await fiber.dispose()
  })
