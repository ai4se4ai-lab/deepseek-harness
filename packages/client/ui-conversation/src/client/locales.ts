...
export const zh = {
  ...
  'fileOpen.unavailableFile': '此部署无法用桌面应用打开文件。该文件的内容显示在聊天中对应的写入/编辑条目里——展开该行即可查看。',
  'fileOpen.unavailableFolder': '此部署无法打开文件夹。工作区中的文件会显示在其各自的条目中。',
...
export const en = {
  ...
  'fileOpen.unavailableFile': 'This deployment can’t open files in a desktop app. This file’s content is shown in its Write/Edit entry in the chat — expand that row to view it.',
  'fileOpen.unavailableFolder': 'This deployment can’t open folders. Files in the workspace show in their own entries.',
