...
The owner payload is `ToolCallOwnerProps`: `callId`, `toolName`, the frozen `block`, optional `cwd` and `home`, and plain `openFile`/`inspect` callbacks. Path summaries relativize to the session cwd first, then replace a leftover POSIX host home with `~`; `filePath` and Host open keep the authored filesystem path. `ToolCallTree` gates `openFile` on `isLoopback && hostDescription().canOpenPath` before any registrant sees it: `undefined` on a deployment that cannot hand a path to a native application, so a file-tool view renders its path as plain text rather than a link that would only fail — the whole row still expands to that Tool's own content/diff. The registration receives the normal session slot runtime share. It does not receive React nodes, Runtime services, or root/subcall knowledge.
