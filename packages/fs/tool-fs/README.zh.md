...
| 工具 | 参数 | 行为 |
|---|---|---|
| `read` | `file_path`、`offset?`、`limit?` | 带行号的 UTF-8 内容与分页 footer；`offset` 从 1 开始，`limit` 默认为配置的 `readLimit`，上限也为该值。后端拒绝为非 UTF-8 的 PDF 会回退为提取其文本层，因此 `.pdf` 读起来像 `.md`；扫描件或无法解析的 PDF 返回 `FS_NOT_TEXT` 并说明原因 |
...
| `readExtractMaxBytes` | `26214400` | `read` 回退为从非 UTF-8 文件（PDF）提取文本层时的字节上限；更大的文件会被拒绝并报 `FS_TOO_LARGE`。 |
| `readExtractMaxChars` | `400000` | 提取文本层的字符上限；超出后窗口以明确的截断行结束。 |
...
- **`read` 只处理 UTF-8 文本与 PDF 文本层**：图像使用独立的 `read_image` 工具；音频和视频仍延期处理。目录目标为 `FS_NOT_REGULAR_FILE`。
- **PDF 提取仅限文本层**：通过内置的 `unpdf`（pdf.js）实现，不是 OCR：扫描件或纯图像 PDF 不返回文本。不提取逐字形 CID 字体解码、表单（XFA）数据，以及 `.docx` 等非 PDF 办公格式。
