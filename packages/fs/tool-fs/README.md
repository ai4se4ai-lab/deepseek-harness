...
|---|---|---|
| `read` | `file_path`, `offset?`, `limit?` | Line-numbered UTF-8 content with a pagination footer; `offset` is 1-based and `limit` defaults to and caps at the configured `readLimit`. A file the backend refuses as non-UTF-8 is retried as a document text layer when it is a PDF, so a `.pdf` reads like a `.md`; a scanned or unparsable PDF returns `FS_NOT_TEXT` with the reason |
...
- **`read` handles UTF-8 text and PDF text layers only** — images use the separate `read_image` tool; audio and video remain deferred. A directory target is `FS_NOT_REGULAR_FILE`.
