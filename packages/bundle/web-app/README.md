...
By default the GUI accepts connections from this machine only. `--host 0.0.0.0` binds all network interfaces so browsers on the LAN (or behind a reverse proxy, e.g. the MindPortalix DSH proxy) can reach it, and the printed URL then includes a LAN address; because this GUI has no login of its own and every request that passes the `/api` Host/Origin fence gets full tool execution, `--host 0.0.0.0` is refused unless `--allow-all-interfaces` is also passed — an explicit acknowledgement that the deployment puts its own authentication and network isolation in front of the port (for example, a reverse proxy on an isolated Docker network). `--trusted-host` adds extra hosts in either binding mode. Host and Origin checks control reachability, while the token exchange authenticates every Host API method and WebSocket stream. The LAN addresses are sampled once at startup, so a network change later is not picked up — restart the GUI to re-advertise.
...
| [`src/startup.ts`](src/startup.ts) | The `web-startup` provider: `--host`, `--port`, `--trusted-host`, `--no-open`, `--allow-all-interfaces`, `--help` |
...
- **Binding all network interfaces requires an explicit opt-in** — `--host 0.0.0.0` is rejected at startup unless `--allow-all-interfaces` is also passed, since this GUI has no login of its own; pass it only behind a reverse proxy or other network isolation that already authenticates access.
