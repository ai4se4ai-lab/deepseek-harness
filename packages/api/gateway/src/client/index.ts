...
export interface RemoteHostFacts {
   ...
   * see `ConnectionHandle.settingsAvailable`). Always equals `isLoopback` on
   * unmodified upstream DSH. Native-path-open features must keep reading
   * `isLoopback` instead — this field says nothing about running on the same
   * machine.
   */
  ...
  get $host(): RemoteHostFacts {
    ...
    if (this.hostFacts === undefined || this.hostFacts.home !== home) {
      this.hostFacts = {
        ...
        settingsAvailable: this.connection.settingsAvailable,
      }
    }
